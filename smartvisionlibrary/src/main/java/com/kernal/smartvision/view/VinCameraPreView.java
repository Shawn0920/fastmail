...
public class VinCameraPreView extends SurfaceView implements SurfaceHolder.Callback,Camera.PreviewCallback{
    ...
    private void finishRecogEvent(){
        ...
        mOnTextChangeListener.onTextChange(recogResult);
//        if (ocrType == 1){//vin
...
//        }
    ...
    public VinCameraPreView(Context context ) {
        super(context);
        mContext = context ;
        mOuter = new WeakReference<SmartvisionCameraActivity>((SmartvisionCameraActivity) context);
        mHolder = getHolder();
...
//          CameraSetting.getInstance(mContext.getApplicationContext()).startPreview(mCamera,VinCameraPreView.this);
    ...
    public void resumeRecogn(){
        finishRecog = false;
    }

    public void finishRecogn(){
        finishRecog = true;
    }
    ...
    public interface OnTextChangeListener{
        void onTextChange(String result);
    }
